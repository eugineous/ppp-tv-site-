...
export default async function MoviesPage() {
  ...
  const reviews = movies.filter(a => a.sourceName?.includes('Empire') || a.sourceName?.includes('RogerEbert') || a.sourceName?.includes('IndieWire')).slice(0, 20);
  const news = movies.filter(a => a.sourceName?.includes('Collider') || a.sourceName?.includes('Screen Rant') || a.sourceName?.includes('SlashFilm') || a.sourceName?.includes('CinemaBlend')).slice(0, 20);
  const global = movies.filter(a => a.sourceName?.includes('IGN')).slice(0, 20);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {movies.length > 0 && <CategoryRow label="Movies & Film" articles={movies.slice(0, 20)} seeAllHref="/movies" accentColor="#E50914" />}
        ...
        {movies.length > 20 && <CategoryRow label="More Movies" articles={movies.slice(20)} seeAllHref="/movies" accentColor="#333" />}
      </div>
    </div>
  );
}
