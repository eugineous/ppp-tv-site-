...
export default async function HealthPage() {
  ...
  const kenya = health.filter(a => a.sourceName?.includes('Nation') || a.sourceName?.includes('Standard')).slice(0, 12);
  const global = health.filter(a => a.sourceName?.includes('WHO') || a.sourceName?.includes('Medical News') || a.sourceName?.includes('Healthline') || a.sourceName?.includes('WebMD') || a.sourceName?.includes('Science Daily')).slice(0, 12);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {health.length > 0 && <CategoryRow label="Health & Wellness" articles={health.slice(0, 12)} seeAllHref="/health" accentColor="#00FF94" />}
        ...
        {health.length > 12 && <CategoryRow label="More Health" articles={health.slice(12)} seeAllHref="/health" accentColor="#333" />}
      </div>
    </div>
  );
}
