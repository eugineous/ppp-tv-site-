...
export default async function LifestylePage() {
  const [lifestyle, health, ent, music, tech, news] = await Promise.all([
    ...
    fetchArticles({ category: 'Technology', limit: 10 }),
    fetchArticles({ category: 'News', limit: 10 }),
  ]);

  const hero = lifestyle.length > 0 ? lifestyle.slice(0, 5) : ent.slice(0, 5);
  const kenya = lifestyle.filter(a => a.sourceName?.includes('Standard') || a.sourceName?.includes('Nation') || a.sourceName?.includes('Pulse') || a.sourceName?.includes('Tuko')).slice(0, 20);
  const africa = lifestyle.filter(a => a.sourceName?.includes('BellaNaija')).slice(0, 20);
  const global = lifestyle.filter(a => a.sourceName?.includes('Vogue') || a.sourceName?.includes('Elle')).slice(0, 20);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {lifestyle.length > 0 && <CategoryRow label="Lifestyle" articles={lifestyle.slice(0, 20)} seeAllHref="/lifestyle" accentColor="#00FF94" />}
        ...
        {lifestyle.length > 20 && <CategoryRow label="More Lifestyle" articles={lifestyle.slice(20)} seeAllHref="/lifestyle" accentColor="#333" />}
      </div>
    </div>
  );
}
