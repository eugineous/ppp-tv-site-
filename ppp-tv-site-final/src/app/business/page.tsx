...
export default async function BusinessPage() {
  const [business, tech, news, politics] = await Promise.all([
    ...
    fetchArticles({ category: 'Politics', limit: 10 }),
  ]);

  const hero = business.length > 0 ? business.slice(0, 5) : news.slice(0, 5);
  const kenya = business.filter(a => a.sourceName?.includes('Business Daily') || a.sourceName?.includes('Standard Business') || a.sourceName?.includes('Nation Business') || a.sourceName?.includes('Capital FM Business')).slice(0, 20);
  const africa = business.filter(a => a.sourceName?.includes('EA Business') || a.sourceName?.includes('Financial Times')).slice(0, 20);
  const global = business.filter(a => a.sourceName?.includes('Bloomberg') || a.sourceName?.includes('Wall Street') || a.sourceName?.includes('CNBC')).slice(0, 20);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {business.length > 0 && <CategoryRow label="Business & Economy" articles={business.slice(0, 20)} seeAllHref="/business" accentColor="#FFE600" />}
        ...
        {business.length > 20 && <CategoryRow label="More Business" articles={business.slice(20)} seeAllHref="/business" accentColor="#333" />}
      </div>
    </div>
  );
}
