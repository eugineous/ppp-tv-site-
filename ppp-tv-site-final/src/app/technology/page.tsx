...
export default async function TechnologyPage() {
  const [tech, science, business, news, lifestyle] = await Promise.all([
    ...
    fetchArticles({ category: 'Lifestyle', limit: 10 }),
  ]);

  const hero = tech.length > 0 ? tech.slice(0, 5) : news.slice(0, 5);
  const africa = tech.filter(a => a.sourceName?.includes('Techweez') || a.sourceName?.includes('TechCabal') || a.sourceName?.includes('HumanIPO') || a.sourceName?.includes('Disrupt Africa')).slice(0, 20);
  const global = tech.filter(a => a.sourceName?.includes('TechCrunch') || a.sourceName?.includes('Verge') || a.sourceName?.includes('Ars Technica') || a.sourceName?.includes('Wired') || a.sourceName?.includes('Engadget')).slice(0, 20);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {tech.length > 0 && <CategoryRow label="Technology" articles={tech.slice(0, 20)} seeAllHref="/technology" accentColor="#FFE600" />}
        ...
        {tech.length > 20 && <CategoryRow label="More Technology" articles={tech.slice(20)} seeAllHref="/technology" accentColor="#333" />}
      </div>
    </div>
  );
}
