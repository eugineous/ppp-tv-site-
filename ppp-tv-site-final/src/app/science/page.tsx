...
export default async function SciencePage() {
  ...
  const space = science.filter(a => a.sourceName?.includes('NASA') || (a.title + ' ' + a.excerpt).toLowerCase().includes('space') || (a.title + ' ' + a.excerpt).toLowerCase().includes('nasa') || (a.title + ' ' + a.excerpt).toLowerCase().includes('planet')).slice(0, 20);
  ...
  const research = science.filter(a => a.sourceName?.includes('Nature') || a.sourceName?.includes('Scientific American') || a.sourceName?.includes('New Scientist')).slice(0, 20);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {science.length > 0 && <CategoryRow label="Science & Discovery" articles={science.slice(0, 20)} seeAllHref="/science" accentColor="#00CFFF" />}
        ...
        {science.length > 20 && <CategoryRow label="More Science" articles={science.slice(20)} seeAllHref="/science" accentColor="#333" />}
      </div>
    </div>
  );
}
