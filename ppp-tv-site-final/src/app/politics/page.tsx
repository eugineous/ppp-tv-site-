...
export default async function PoliticsPage() {
  ...
  const kenya = politics.filter(a => a.sourceName?.includes('Nation') || a.sourceName?.includes('Standard') || a.sourceName?.includes('Star') || a.sourceName?.includes('Capital') || a.sourceName?.includes('Citizen') || a.sourceName?.includes('KBC')).slice(0, 12);
  ...
  const africa = eastAfrica.filter(a => a.sourceName?.includes('Monitor') || a.sourceName?.includes('Citizen Tanzania') || a.sourceName?.includes('Vanguard') || a.sourceName?.includes('Premium Times') || a.sourceName?.includes('Joy Online')).slice(0, 12);
  const breaking = news.filter(a => a.sourceName?.includes('Nation') || a.sourceName?.includes('Standard') || a.sourceName?.includes('Star')).slice(0, 12);

  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      {hero.length > 0 && <HeroBanner articles={hero} />}
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        {politics.length > 0 && <CategoryRow label="Kenya Politics" articles={politics.slice(0, 12)} seeAllHref="/politics" accentColor="#FF4500" />}
