...
export default async function TrendingPage() {
  ...
  const news = recent.filter(a => a.category === 'News').slice(0, 20);
  const politics = recent.filter(a => a.category === 'Politics').slice(0, 20);
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', paddingTop: '1rem' }}>
        ...
        {recent.length > 0 && <CategoryRow label="🔥 Latest Stories" articles={recent.slice(0, 20)} seeAllHref="/trending" accentColor="#FF007A" />}
        ...
        {recent.length > 20 && <CategoryRow label="More Stories" articles={recent.slice(20)} seeAllHref="/trending" accentColor="#555" />}
      </div>
    </div>
  );
}
