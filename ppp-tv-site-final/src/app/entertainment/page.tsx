...
export default async function EntertainmentPage() {
  const [ent, music, movies, lifestyle, sports] = await Promise.all([
    ...
    fetchArticles({ category: 'Movies', limit: 12 }),
    fetchArticles({ category: 'Lifestyle', limit: 12 }),
    fetchArticles({ category: 'Sports', limit: 8 }),
  ]);

  const hero = ent.slice(0, 5);
  const kenya = ent.filter(a => ['SDE Kenya','Ghafla Kenya','Mpasho','Pulse Live Kenya','Tuko Kenya'].some(s => a.sourceName?.includes(s.split(' ')[0]))).slice(0, 12);
  ...
  const africa = ent.filter(a => a.sourceName?.includes('Pulse Nigeria') || a.sourceName?.includes('Pulse Ghana') || a.sourceName?.includes('This Is Africa')).slice(0, 12);
  const global = ent.filter(a => a.sourceName?.includes('Variety') || a.sourceName?.includes('Deadline') || a.sourceName?.includes('Hollywood Reporter') || a.sourceName?.includes('Entertainment Weekly')).slice(0, 12);
