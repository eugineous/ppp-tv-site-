...
async function fetchRSSFeed(feed: { url: string; name: string; category: string }): Promise<Article[]> {
  try {
    ...
    const items: Array<{ title: string; link: string; pubDate: string; description: string; mediaUrl: string }> = [];
    ...
    for (const match of itemMatches) {
      ...
      items.push({ title, link, pubDate, description, mediaUrl });
    ...
    const articles = await Promise.all(
      items.map(async (item): Promise<Article> => {
        ...
        const content = scraped.content || `<p>${excerpt}</p>`;
