...
- [-] 2. Rewrite the Cloudflare Worker — Supabase client and data layer
  - [x] 2.1 Update `Env` interface and add Supabase client helpers in `worker/index.ts`
...
- [-] 3. Rewrite the Cloudflare Worker — AI pipeline functions
  - [x] 3.1 Implement `detectLanguageHeuristic` and `detectAndTranslate` in `worker/index.ts`
  ...
  - [x] 3.3 Implement `validateGeminiOutput` and `rewriteWithGemini` in `worker/index.ts`
...
- [ ] 4. Rewrite the Cloudflare Worker — batch processor and cron handler
  - [x] 4.1 Implement `processArticleBatch` in `worker/index.ts`
  ...
  - [x] 4.3 Update the `scheduled` handler in `worker/index.ts`
...
- [ ] 6. Rewrite the Cloudflare Worker — API endpoints and RSS feed pivot
  - [x] 6.1 Update RSS feed list in `worker/index.ts` to Entertainment/Sports/Technology only
  ...
  - [x] 6.2 Update `/articles` endpoint to read from Supabase
...
- [ ] 7. Rewrite the Cloudflare Worker — security headers and rate limiting
  - [x] 7.1 Add `SECURITY_HEADERS` constant and apply to all responses
  ...
  - [x] 7.2 Implement in-memory rate limiter and apply to all endpoints
...
- [ ] 9. Update Next.js frontend — next.config.js
  - [x] 9.1 Add redirects for removed category pages in `next.config.js`
    - Add `async redirects()` returning 301 redirects: `/politics` → `/`, `/news` → `/`, `/health` → `/`, `/science` → `/`, `/business` → `/`
    - _Requirements: 4.2, 4.6_

  - [x] 9.2 Update security headers in `next.config.js`
...
- [ ] 10. Update Next.js frontend — navigation components
  - [x] 10.1 Update `Header.tsx` navigation
  ...
  - [x] 10.2 Update `MobileMenu.tsx` navigation links
  ...
  - [x] 10.3 Update `Footer.tsx` Browse column
...
- [x] 11. Update Next.js frontend — delete removed category pages
...
- [ ] 12. Update Next.js frontend — category pages and homepage
  - [x] 12.1 Update `src/app/entertainment/page.tsx`
  ...
  - [x] 12.2 Update `src/app/sports/page.tsx`
  ...
  - [x] 12.3 Update `src/app/technology/page.tsx`
  ...
  - [x] 12.4 Update `src/app/page.tsx` homepage
...
- [ ] 13. Update Next.js frontend — article display and worker lib
  - [x] 13.1 Update `src/lib/worker.ts` to support `subcategory` filter
    - Add `subcategory` to `FetchArticlesOptions` and pass it as a query param to the Worker
    - _Requirements: 4.8, 9.3_

  - [x] 13.2 Update article detail page (`src/app/news/[slug]/page.tsx`) to display new Supabase fields
